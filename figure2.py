...
for idx, loc in enumerate(locs):
	...
	for chan in chans:
		...
		plt.semilogx(f, pcor, label='Corr ' + tr.stats.station + ' ' + tr.stats.location + ' ' + chan)
		plt.semilogx(f,p, label='Raw ' + tr.stats.station + ' ' + tr.stats.location + ' ' + chan, alpha=0.5, linestyle='dotted', linewidth=2.5)
	per, nlnm = get_nlnm()
	per, nhnm = get_nhnm()
	plt.semilogx(1./per, nlnm, color='k', linewidth=2)
	plt.semilogx(1./per, nhnm, color='k', linewidth=2, label='NLNM/NHNM')
	plt.axvspan(pmin, pmax, facecolor='g', alpha=0.3)
	plt.xlabel('Frequency (Hz)')
	plt.ylabel('Power (dB rel. 1 $(m/s^2)^2/Hz$)', fontsize=16)
	plt.xlim((1./(1000.), 1./2.))
	plt.ylim((-190, -90))
	plt.legend(loc=9, ncol=2, fontsize=16)
	if idx == 0:
		plt.text(1./2600., -90, '(a)')
	else:
		plt.text(1./2600., -90, '(b)')
plt.savefig('PSD_example.png', format='PNG', dpi=200)
plt.show()
